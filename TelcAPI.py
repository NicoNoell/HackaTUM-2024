...
class Runner():
    def getScenario(scenarioId:str) -> Scenario:
        try:
            return Scenario(
                requests.get(f"http://localhost:8090/Scenarios/get_scenario/{scenarioId}").json()
            )
        except:
    ...
    def initScenario(scenario:Scenario) -> None:
        try:
            requests.post(f"http://localhost:8090/Scenarios/initialize_scenario", json=scenario.json(), headers=PAYLOAD_HEADER).json()
        except:
            print("Response ist None oder ungültig")

    def updateScenario(scenarioId:str) -> None:
        try:
            requests.put(f"http://localhost:8090/Scenarios/update_scenario/{scenarioId}").json()
        except:
            print("Response ist None oder ungültig")

    def launchScenario(scenarioId) -> None:
        try:
            requests.post(f"http://localhost:8090/Runner/launch_scenario/{scenarioId}").json()
        except:
            print("Response ist None oder ungültig")
...
class Backend():
    ...
    def getAllCustomers(scenarioId:str) -> list[Customer]:
        try:
            return [Customer(c) for c in requests.get(f"http://localhost:8080/scenarios/{scenarioId}/customers").json()]
        except:
            print("Response ist None oder ungültig")
    ...
    def getScenarioMetadata(scenarioId:str) -> None:
        try:
            return requests.get(f"http://localhost:8080/scenario/{scenarioId}/metadata").json()
        except:
            print("Response ist None oder ungültig")

    def createScenario(numberOfVehicles:int = 5, numberOfCustomers:int = 5) -> Scenario:
        try:
            return Scenario(
                requests.post(f"http://localhost:8080/scenario/create?numberOfVehicles={numberOfVehicles}&numberOfCustomers={numberOfCustomers}").json()
            )
        except:
            print("Response ist None oder ungültig")
    ...
    def getAllScenarios() -> list[Scenario]:
        try:
            return [Scenario(s) for s in requests.get(f"http://localhost:8080/scenarios").json()]
        except:
            print("Response ist None oder ungültig")
    ...
    def getAllVehicles(scenarioId:str) -> list[Vehicle]:
        try:
            return [Vehicle(v) for v in requests.get(f"http://localhost:8080/scenarios/{scenarioId}/vehicles").json()]
        except:
            print("Response ist None oder ungültig")
